const fs = require('module').createRequire(require.main.filename)("fs");

let adminStr = fs.readFileSync('routes/adminRoutes.js', 'utf8');
...
if (!adminStr.includes('/restaurant-requests')) {
    adminStr = adminStr.replace('module.exports = router;', replacementStr);
    
    // Check if Restaurant model is imported in adminRoutes.js
    if (!adminStr.includes('const Restaurant = ')) {
        adminStr = adminStr.replace('const User = require("../models/User");', 'const User = require("../models/User");\nconst Restaurant = require("../models/Restaurant");');
    }
    
    fs.writeFileSync('routes/adminRoutes.js', adminStr);
